...
export class AnimalController {
    ...
    public async cadastrar(req: NextRequest) {
        ...
        try{
            ...
            const mensagem = error instanceof Error ? error.message : "Não foi possivel concluir o cadastro do animal devido a um erro inesperado";
    ...
    public async buscarCompleto(req: NextRequest, params: { id_animal: string}) {
        ...
        try {
            ...
            const animal = this.animalService.buscarAnimalCompleto(id_animal);

            if(!animal){
                return NextResponse.json({ error: "Nenhum animal foi encontrado com o ID especifico."}, { status: 400 })
            } 
            ...
            return NextResponse.json(this.animalService, { status: 500});
